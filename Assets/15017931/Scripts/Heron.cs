...
public class Heron : MonoBehaviour {
	...
	void Start () {
		...
		myAnim ["Walk"].speed = walkSpeed;
		myAnim ["Run"].speed = runSpeed;
		myAnim ["FishingWalk"].speed = fishWalkSpeed;
		...
		while (true) {
			...
			if (status == HeronStatus.Walking && offsetMoveDirection.magnitude > 0.01f) {
				if (!fishing) {
					...
					myAnim.CrossFade ("Walk", 0.4f);
				} else {
					targetSpeed = fishWalkAnimSpeed * fishWalkSpeed;
					myAnim.CrossFade ("FishingWalk", 0.4f);
				}
			} else if (status == HeronStatus.Running) {
				targetSpeed = runAnimSpeed * runSpeed;
				myAnim.CrossFade ("Run", 0.4f);
			} else {
				if (!fishing) {
					targetSpeed = 0;
					if (!strechNeck)
						myAnim.CrossFade ("IdleHold", 0.4f);
					else
						myAnim.CrossFade ("IdleStrechNeck", 0.4f);
				} else {
					targetSpeed = 0;
					myAnim.CrossFade ("IdleFishing", 0.4f);
				}
			}
