    ...
    public override void UpdateState(MyAnky _owner)
    {
        ...
        if(_owner.myStats.hunger < 50)
            ...
            if (_owner.gameObject.transform.position.y < 52 || _owner.gameObject.transform.position.y > 94)
            {
                ...
                _owner.waterTarget.transform.position = _owner.aStarScript.mapGrid.getFoodSource(_owner.gameObject).position;
                _owner.aStarScript.target = _owner.waterTarget.gameObject;

                Debug.Log(_owner.aStarScript.mapGrid.getFoodSource(_owner.gameObject).position);
        ...
        else if (_owner.myStats.thirst < 50)
        {
            ...
            if (_owner.gameObject.transform.position.y > 36)
            {
                ...
                _owner.aStarScript.target = _owner.gameObject;
                _owner.waterTarget.transform.position = _owner.aStarScript.mapGrid.getWaterEdge(_owner.gameObject).position;
                _owner.aStarScript.target = _owner.waterTarget.gameObject;

                Debug.Log(_owner.aStarScript.mapGrid.getWaterEdge(_owner.gameObject).position);
